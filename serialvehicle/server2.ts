...
if (arduino?.path) {
  ...
  console.log("✓ Arduino bulundu:", arduino.path);
} else console.warn("Arduino bulunamadı");

let nanoSerial: SerialPort | null = null;
if (nano?.path) {
  nanoSerial = new SerialPort({ path: nano.path, baudRate: 9600 });
  console.log("✓ Nano bulundu:", nano.path);
} else console.warn("Nano bulunamadı");
...
// Server start
httpServer.listen(8082, () => console.log("✓ Server 8082 portunda"));
